    ...
    def install(self, APK_path=None, verbose=True, reinstall=False):
        try:
            ...
            if not installed or reinstall:
                if APK_path is None:
                    # Source checkout and installed ROS package both carry the
                    # Quest APK; resolve it without assuming the working directory.
                    source_apk = Path(__file__).resolve().parents[1] / 'APK' / 'teleop-debug.apk'
                    if source_apk.is_file():
                        APK_path = str(source_apk)
                    else:
                        from ament_index_python.packages import get_package_share_directory
                        APK_path = str(Path(get_package_share_directory('oculus_reader')) / 'APK' / 'teleop-debug.apk')
